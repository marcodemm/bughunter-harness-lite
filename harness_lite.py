    ...
    while True:
        ...
        if _looks_like_target(objective) and " " not in objective.strip():
            ...
            objective = (
                ...
                f"(6) Only THEN call finish() with a SHORT free-form "
                f"summary: 'Stack: <what you fingerprinted, with version "
                f"only if literally seen in a header or tool output — DO "
                f"NOT invent one>. Notable: <one line — a specific header, "
                f"cookie, banner or 200 OK that stood out; else \"none\">. "
                f"Suspected finding: <one line or none>.' Keep it under "
                f"3 lines. The harness will auto-classify every probed "
                f"path by HTTP status in the REPORT.md separately — do "
                f"NOT reproduce that list in your summary."
            )
